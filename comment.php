...
$state->execute();
 ...
 $query = "SELECT * FROM comments WHERE id2 = {$_GET['id']}";
 ...
 if (isset($_POST['submit'])) {
    ...
    {
        $message = "<strong>Incorrect verification code. Retry...</strong>'";
    }
    
    else{
    $query = "INSERT INTO comments (comment, commentBy,id2) values (:comment, :commentBy,:id2)";

   $statement = $db->prepare($query);
   $statement->bindValue(':comment', $comment);        
   $statement->bindValue(':commentBy', $user);
...
>
                ...
                <div class="card-body">
                    ...
                    <!------------The form to make some commmants started now------->
                    <form method="post" >
                    <div class = "form-group">
                        <label><h3>Comments:</h3></label><br>
                        <input type="text" class="form-control" name="comments" ><br>
                        </div>

                        <div class = "form-group">
                        <label><img src="captcha.php" alt=""></label><br>
                        <input type="text" class="form-control" name="captcha" ><br>
                        </div>
                        <button class="btn-primary" type="submit" name="submit">Comment</button><br>
                   
                    </form>
                    <h2><?php echo $message; ?></h2>
                    <br>
                    <h3>Here is your comments: </h3><br>
                    <?php foreach ($p as $posts): ?>
                    <ul>
                   <li> <h4><?= $posts['Comment']; ?></h4></li>
                   </ul >
                     <?php endforeach; ?>
